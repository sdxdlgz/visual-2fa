...
- 📷 **多种导入方式**：摄像头扫描、二维码图片、`otpauth://` URI、Base32 密钥
- ⏱️ **完整 OTP 支持**：TOTP / HOTP、SHA-1 / SHA-256 / SHA-512、6–8 位、自定义周期与计数器
- 🗂️ **日常管理**：备注、分组、标签、收藏、搜索、筛选、排序、最近使用
...
二维码图片只在浏览器内解析，不会上传原图。当前支持标准单条 `otpauth` 二维码，不支持 Google Authenticator 的批量 `otpauth-migration://` 二维码。
...
测试覆盖 RFC 4226/6238 向量、客户端密钥包装/解包、条目 AEAD 加解密、主密码哈希与基础认证原语。
